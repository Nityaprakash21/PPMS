...
function populateTable(data) {
    ...
    // Assuming data.tankdata is an array of tank objects
...
// Function to populate the dropdowns with tank IDs and dispensing unit numbers
function populateDropdown(data) {
    const tankDropdown = document.getElementById("tankid");
    const dispensingUnitDropdown = document.getElementById("dispensingunitnumber");

    // Clear existing options
    tankDropdown.innerHTML = "";
    dispensingUnitDropdown.innerHTML = "";

    // Populate tank ID dropdown
    data.tankdata.forEach(tank => {
        const option = document.createElement("option");
        option.value = tank.tank_id;
        option.text = tank.tank_id;
        tankDropdown.appendChild(option);
    });

    // Populate dispensing unit number dropdown with fixed values (1, 2, 3, 4)
    const dispensingUnitValues = [1, 2, 3, 4];
    dispensingUnitValues.forEach(value => {
        const option = document.createElement("option");
        option.value = value;
        option.text = value;
        dispensingUnitDropdown.appendChild(option);
    });
}

// Call the function to fetch user data and populate the table and dropdowns
