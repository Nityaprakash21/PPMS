...
function populateTable(data) {
    ...
    data.tankdata.forEach(tank => {
        ...
        row.innerHTML = `
            <td>${tank.tank_id}</td>
            <td>${tank.tank_fuel}</td>
            <td>${tank.tank_capacity}</td>
            <td>${tank.optimum_volume}</td>
            <td>${tank.tank_name}</td>
            <td>${tank.minimum_volume}</td>
        `;
...
function populateDropdown(data) {
    const dropdown = document.getElementById("tankfuel");
    dropdown.innerHTML = "";

    // Assuming data.ftdata is an array of fuel objects
    data.ftdata.forEach(fuel => {
        const option = document.createElement("option");
        option.value = fuel.fuel_name;
        option.text = fuel.fuel_name;
