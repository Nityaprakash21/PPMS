...
function populateFuelDropdown(data) {
    ...
    const workingStatusDropdown = document.getElementById("duworkingstatus");

    dropdown.innerHTML = "";
    workingStatusDropdown.innerHTML = "";
    ...
    // Populate dispensing working status dropdown with fixed values
    const workingStatusValues = ["Operational", "Maintenance", "Out of Service"];
    workingStatusValues.forEach(status => {
        const option = document.createElement("option");
        option.value = status;
        option.textContent = status;
        workingStatusDropdown.appendChild(option);
    });
}
