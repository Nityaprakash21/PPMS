...
function populateTable() {
    ...
    fetch('/get-user-data')
      ...
      .then(userData => {
        if (userData) {
          // Assuming userData has a property called 'fidata' which is an array of fuel inventory data
          userData.fidata.forEach(tank => {
            const row = document.createElement("tr");
            row.innerHTML = `
              <td>${tank.fuel_inventory_id}</td>
              <td>${tank.station_id}</td>
              <td>${tank.tank_id}</td>
              <td>${tank.fuel_type}</td>
              <td>${tank.quantity_available}</td>
