...
function populateTable() {
    ...
    fetch('/get-user-data')
      ...
      .then(userData => {
        if (userData) {
          // Assuming userData has an opdata property
          userData.opdata.forEach(tank => {
            const row = document.createElement("tr");
            row.innerHTML = `
              <td>${tank.operator_id}</td>
              <td>${tank.operator_name}</td>
              <td>${tank.joining_date}</td>
              <td>${tank.date_of_birth}</td>
              <td>${tank.salary}</td>
              <td>${tank.contact_details}</td>
            `;
