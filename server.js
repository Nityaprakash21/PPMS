...
app.use(session({
  ...
  app.post('/du', async (req, res) => {
    ...
    // Check if the request body contains the required fields
    if (!req.body.duid || !req.body.dufuel || !req.body.duworkingstatus || !req.body.duworkingstate || !req.body.machineid || !req.body.tankid) {
      console.log('Missing required fields in request body');
      return res.status(400).send('Missing required fields in request body');
    }
  
    const { duid, dufuel, duworkingstatus, duworkingstate, machineid, tankid } = req.body;
  ...
  app.post('/fuel', async (req, res) => {
    ...
    // Check if the request body contains the required fields
    if (!req.body.date || !req.body.fuelname || !req.body.fueldensity ) {
      console.log('Missing required fields in request body');
      return res.status(400).send('Missing required fields in request body');
    }
  
    const { date, fuelname, fueldensity } = req.body;
    ...
    if (!user) {
      ...
      console.log(user); // Log the user to verify that it is correct
      return res.sendFile(path.join(__dirname, 'public', 'fuel.html'));
    }
  });
